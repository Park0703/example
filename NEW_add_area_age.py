# -*- coding: utf-8 -*-
"""
Created on Sun Jul 18 19:29:03 2021
...
####### age DF 전처리
#10대미만, 10대, 90대, 100대 남녀데이터 삭제.
age_pp = age.drop(["10대미만(여자)", "10대미만(남자)", "10대(여자)", "10대(남자)", "90대(여자)", "90대(남자)",
                "100대(여자)", "100대(남자)"], axis=1)

#합체.
train_agg=pd.merge(train_agg,age_pp, on='지역')
test_agg=pd.merge(test_agg,age_pp, on='지역')

#컬럼확인하기
train_agg.shape, test_agg.shape #((404, 49), (150, 48))
...
submission.to_csv('NEW_CB.csv', index=False)
...
submission.to_csv('NEW_XGB_add_area_age.csv', index=False)
...
submission.to_csv('NEW_RF_add_area.csv', index=False)
